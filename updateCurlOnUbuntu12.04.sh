...
if [ `lsb_release -a|grep Release|awk '{print $2}'` == '12.04' ]; then
	...
	rm /tmp/myppa.list
	sudo apt-get update
	sudo apt-get -V build-dep curl libc6 libcurl3 libcurl3-gnutls php5-curl apache2 libgnutls28
	sudo apt-get -V install curl libc6 libcurl3 libcurl3-gnutls php5-curl apache2 libgnutls28
else
